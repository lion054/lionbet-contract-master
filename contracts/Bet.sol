...
contract Bet is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Sent when once a bet is cancelled
     */
    event BetCancelled(
        bytes32 _eventId,
        address _player,
    ...
    /**
     * @notice gets a list ids of all already betted events
     * @return pendingEvents the list of pending sport events
     */
    function getBettedEvents()
    ...
    /**
     * @notice read already betted payload about the given event
     * @param _eventId      id of the sport event on which to bet
     */
    function getBetPayload(bytes32 _eventId) public view returns (
        uint8 chosenWinner,
        uint amount
    ) {
        Payload[] storage bets = eventToBets[_eventId];
        for (uint i = 0; i < bets.length; i++) {
            if (bets[i].user == msg.sender) {
                return (bets[i].chosenWinner, bets[i].amount);
            }
        }
        return (0, 0);
    }
    ...
    /**
     * @notice cancels a bet on the given event
     * @param _eventId      id of the sport event on which to bet
     */
    function cancelBet(bytes32 _eventId)
        public
        notAddress0(msg.sender)
        nonReentrant
    {
        ...
        // transfer the contract's account into the player's money
        Payload[] storage bets = eventToBets[_eventId];
        uint found;
        for (uint i = 0; i < bets.length; i++) {
            if (bets[i].user == msg.sender && bets[i].eventId == _eventId) {
                found = i;
                break;
            }
        }
        uint _amount = bets[found].amount;
        payable(msg.sender).transfer(_amount);

        // remove the existing bet
        for (uint j = found + 1; j < bets.length; j++) {
            bets[j - 1] = bets[j];
        }
        bets.pop();
        ...
        for (uint i = 0; i < userBets.length; i++) {
            if (userBets[i] == _eventId) {
                for (uint j = i + 1; j < userBets.length; j++) {
                    userBets[j - 1] = userBets[j];
                }
                userBets.pop();
            }
        }
